...
const defaultInterval = computed(() => {
    if (props.intervals.includes('month')) {
        return 'month';
    }

    return props.intervals[0] ?? 'month';
});
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Create plan', href: route('acp.billing.plans.create') },
];
...
const form = useForm({
    ...
    price: '',
    currency: props.default_currency ?? 'USD',
    description: '',
    features_text: '',
    is_active: true,
});
